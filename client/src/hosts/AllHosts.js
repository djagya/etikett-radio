    ...
    switch (i) {
      ...
      case 1:
        ...
        if (lastSort !== isActive) {
          setHostData(
            [...hostData].sort((hostA, hostB) =>
              hostA.isActive < hostB.isActive ? -1 : 1,
            ),
          );
          setLastSort(1);
        } else {
        ...
        <ol className="all-data host-list-grid">
          ...
          <li className="button-container">
            <Link className="link-button" to={`/user/show/${context.id}`}>
              <button
                type="button"
                onClick={() => context.setEditHostID(host.userID)}>
                edit
              </button>
            </Link>
          </li>
  ...
  return (
    <DocumentTitle title="All Hosts">
      <div className={`all-host-component all-list ${context.gapClass}`}>
        <h1 id="main">all shows.</h1>
        <div className="list-container">
          <Link className="button-container" to={`/user/${context.id}`}>
            <button type="button">back</button>
          </Link>
          <div>
            <ul className="list-header host-list-grid">{renderLiHeader()}</ul>
            {renderHosts()}
          </div>
        </div>
      </div>
