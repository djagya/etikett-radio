  ...
  useEffect(() => {
    ...
    GetData('/host')
      .then((data) => {
        setHostData(
          data.host
            .filter((host) => host.isActive === 'active')
            .sort((hostA, hostB) => (hostA.hostName < hostB.hostName ? -1 : 1)),
        );
        setLoading(false);
      })
      .catch((err) => {
        setLoading(false);
        alert.error(
          'Failed to fetch shows from the server. Please contact the admin.',
        );
      });
  }, [alert]);
  ...
  return (
    <DocumentTitle title="Hosts">
      <div className={`${context.gapClass} host-list-component`}>
        <h1 id="hosts-title">shows.</h1>
        <div className="grid-wrapper">
          <div className=" hosts-list">{renderHost()}</div>
        </div>
      </div>
    </DocumentTitle>
  );
