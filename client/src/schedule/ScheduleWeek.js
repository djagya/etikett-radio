...
export default function ScheduleWeek(data) {
  ...
  if (selected === 'week' && thisWeek !== currWeek) {
    return null;
  }
  if (selected === 'month' && thisMonth !== currMonth) {
    return null;
  }
  if (selected === 'nextMonth' && thisMonth !== nextMonth()) {
    return null;
  }

  const mon = week.filter(
    (data) => moment(data.from).format('dddd') === 'Monday',
  );
  const tue = week.filter(
    (data) => moment(data.from).format('dddd') === 'Tuesday',
  );
  const wed = week.filter(
    (data) => moment(data.from).format('dddd') === 'Wednesday',
  );
  const thu = week.filter(
    (data) => moment(data.from).format('dddd') === 'Thursday',
  );
  const fri = week.filter(
    (data) => moment(data.from).format('dddd') === 'Friday',
  );
  const sat = week.filter(
    (data) => moment(data.from).format('dddd') === 'Saturday',
  );
  const sun = week.filter(
    (data) => moment(data.from).format('dddd') === 'Sunday',
  );
  ...
  const dates = (day) =>
    day.map((data, i) => {
      return <ScheduleEntry data={data} key={i} />;
    });
    ...
    return (
      <ul className="day-dates">
        <li className="day-head">
          <div className={`${isPast} day-header`}>
            {moment(day[0].from).format('dddd')}
          </div>
          <div className={`${isPast}`}>
            {moment(day[0].from).format().substring(0, 10)}
          </div>
        </li>
        {dates(day)}
      </ul>
