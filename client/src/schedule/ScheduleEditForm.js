...
export default function ScheduleEdit(props) {
  ...
  const timeFrom =
    dataFrom.toString().substring(0, 10) +
    'T' +
    dataFrom.toString().substring(11, 16);
  const timeTo =
    dataTo.toString().substring(0, 10) +
    'T' +
    dataTo.toString().substring(11, 16);
  ...
  const handleSubmit = (event) => {
    ...
    postData(`/schedule/${id}`, body).then((data) => {
      reload(data);
    });
  ...
  const handleFormInput = (event) => {
    ...
    switch (id) {
      ...
      case 'to':
        ...
        break;
      default:
        console.log('Schedule Edit ran through without any effect');
    }
  };
