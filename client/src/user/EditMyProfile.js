...
export default function MyProfile(props) {
  ...
  const handlePatch = (body) => {
    ...
    PatchData(`/users/${context.id}`, body).then((data) => {
      if (!data.success) {
        alert.error('Something went wrong...');
      } else {
        ...
        <div>
          ...
          {editPW ? (
            <form className="input-form" onSubmit={handlePWSubmit}>
              <div className="button-container">
                <button type="button" onClick={() => setEditPW(false)}>
                  cancel
                </button>
              </div>
              <div className="grid-container">
                <label htmlFor="pw">
                  ...
                  <input
                    type="password"
                    id="pw"
                    placeholder="At least 8 signs long"
                    value={pw}
                    onChange={handleFormInput}
                  />
                </label>
                <label htmlFor="pwCheck">
                  ...
                  <input
                    type="password"
                    id="pwCheck"
                    placeholder="repeat to check yourself"
                    value={pwCheck}
                    onChange={handleFormInput}
                  />
                </label>
            ...
            <div className="button-container option-button">
              <button type="button" onClick={() => setEditPW(true)}>
                change password
              </button>
            </div>
          )}
